...
LRESULT CALLBACK KeyboardInputController::KeyboardHookProc(int nCode, WPARAM wParam, LPARAM lParam) {
	if (nCode >= 0) {
		...
		if (wParam == WM_KEYDOWN) {
			...
			bool found = false;
			for (int i = 0; i < (int)(sizeof(keys) / sizeof(keys[0])) && !found; i++)
				if (keyCode == *keys[i]) {
					*keyPressed[i] = true;
					found = true;
				}
			...
			UINT keyCode = keyboardInfo->vkCode;
			bool found = false;
			for (int i = 0; i < (int)(sizeof(keyPressed) / sizeof(keyPressed[0])) && !found; i++)
				if (keyCode == *keys[i]) {
					*keyPressed[i] = false;
					found = true;
				}
...
void KeyboardInputController::cameraControl(Camera* camera, float deltaTime) {
	...
	float velocity = camera->getVelocity() * deltaTime;
