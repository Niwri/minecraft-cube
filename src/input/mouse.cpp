...
void MouseInputController::cameraControl(Camera* camera, GLFWwindow* window, int width, int height) {
    ...
    float deltaX = mouseX - (float)width/2.0f;
    float deltaY = mouseY - (float)height/2.0f;

    deltaX *= sensitivity;

    deltaY *= sensitivity * (float)width / (float)height; // Multiplies deltaY with a ratio to maintain consistency with deltaX
